...
const INTERVIEW_CRITERIA = {
    ...
    'College of Computing Studies': [
        { id: 'logic',   label: 'Logical & Analytical Thinking', desc: 'Problem decomposition, pattern recognition, and reasoning', max: 5 },
        { id: 'techint', label: 'Technology Interest',           desc: 'Curiosity and enthusiasm for computing and software',       max: 5 },
    ],
    'College of Engineering': [
        { id: 'mathap',  label: 'Mathematical Aptitude',     desc: 'Comfort with quantitative and spatial reasoning',         max: 5 },
        { id: 'techap',  label: 'Technical Problem Solving', desc: 'Applied engineering thinking and design intuition',       max: 5 },
    ],
    'College of Nursing': [
    ...
    'College of Business Administration': [
        ...
        { id: 'bizmind', label: 'Business Mindset',          desc: 'Understanding of commerce, markets, and economics',       max: 5 },
    ],
    'College of Arts and Sciences': [
        { id: 'crit',    label: 'Critical & Creative Thinking', desc: 'Depth of thought, originality, and intellectual curiosity', max: 5 },
    ...
    'College of Islamic and Arabic Studies': [
        { id: 'cultural', label: 'Cultural Appreciation',   desc: 'Understanding of Islamic culture, history, and values',   max: 5 },
        { id: 'lang',    label: 'Language Aptitude',         desc: 'Potential and enthusiasm for Arabic language learning',   max: 5 },
    ],
};
...
const SCHED_KEY = dept => `wmsu_isched_${btoa(unescape(encodeURIComponent(dept))).replace(/=/g, '')}`;
...
// Returns the next { date, session, time } or null if all full
function getNextAvailableSlot(dept) {
    ...
    for (const day of cfg.days) {
        ...
        if ((occ[amKey] || 0) < cfg.amCapacity) return { date: day.date, session: 'AM', time: cfg.amStart };
        if ((occ[pmKey] || 0) < cfg.pmCapacity) return { date: day.date, session: 'PM', time: cfg.pmStart };
    }
    return null; // fully booked
}

// ── Score state (cleared on modal close) ─────────
const _scores = {};
...
function buildDayRow(i, dateVal) {
    return `<div class="sched-day-input-row" id="dayRow${i}">
        ...
        <input type="date" class="sched-day-date-input" id="dayDate${i}" value="${dateVal || ''}">
...
function updateSchedPreview() {
    const days  = document.querySelectorAll('.sched-day-date-input').length;
...
function saveScheduleSetup() {
    ...
    const dateInputs = document.querySelectorAll('.sched-day-date-input');
    const days       = [];
    let hasError     = false;

    dateInputs.forEach((inp, i) => {
        if (!inp.value) {
            alertEl.textContent = `Please select a date for Day ${i + 1}.`;
            alertEl.className   = 'modal-alert show err';
            hasError = true;
        } else {
            days.push({ date: inp.value });
        }
    });
    if (hasError) return;
    if (!days.length) {
        alertEl.textContent = 'Add at least one interview day.';
        alertEl.className   = 'modal-alert show err';
        return;
    }
...
function setDept(dept) {
    ...
    // reset active sidebar link to "all"
        ...
        if (!cfg) {
            ...
            body.innerHTML = `
                ...
                <div class="sched-gate sched-gate-warn">
                    ...
                    <div class="sched-gate-sub">${totalFilled} of ${totalSlots} slots are taken. Add more days or increase slot capacity to accommodate more applicants.</div>
            ...
            body.innerHTML = `
                ...
                <div class="auto-slot-card">
                    <div class="auto-slot-top">
                        ...
                        <span>Auto-assigned Slot · Day ${dayIndex}</span>
                ...
                <p class="auto-slot-note">Slot assigned automatically — the next open position in your configured schedule. The applicant will be notified of their date and time.</p>
                <div class="form-field">
...
function confirmAction() {
    ...
    if (activeAction === 'interview') {
        ...
        showToast(`${a.name} → ${dateLabel} · ${slot.session} · ${slot.time}`, 'info');
        return;
    }
...
function init() {
    ...
    setInterval(() => { renderKPIs(); render(); renderScheduleCard(); }, 15000);
}
