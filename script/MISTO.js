...
function modalStep(dir) {
    if (dir === 1) {
        if (modalCurrentStep === 1) {
            ...
            verifiedEmployee = found.name
                ? found
                : { name: found.name || 'Unknown', initials: (found.name || '??').split(' ').map(w => w[0]).slice(0, 2).join(''), email: found.email || '—', position: found.position || '—' };
...
function addRoom() {
    ...
    ROOMS.push({
        ...
        dept:     document.getElementById('roomDept').value || 'General Use',
    });
...
function filterFaculty() {
    ...
    renderFaculty(FACULTY.filter(f =>
        !q ||
